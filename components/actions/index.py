...
def extract_credit_card_number(params: dict):
    ...
    data = {
        "model": "gpt-4o-mini",
        "messages": [
            {"role": "user", "content": [{"type": "text", "text": "Extract the text in this image"},
                                         {"type": "image_url", "image_url": {"detail": "low", "url": f"data:image/{ext};base64,{base64_image}"}}]}
        ]
    }
    ...
    response = requests.post(
        ...
        json={"model": "gpt-4o-mini", "messages": [{"role": "system", "content": "get the credit card number from the following text"},
