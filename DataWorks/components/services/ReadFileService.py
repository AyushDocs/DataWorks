...
class ReadFileService:
    ...
    def read_file(file_path):
        ...
        if (
            ...
            logging.error('USER requested file path: %s %s', abs_file_path,'is outside of /data directory')
            return {"error": "File requested is outside of data directory"}, 401
        if not os.path.exists(abs_file_path):
            logging.error('USER requested file path: %s %s', abs_file_path,'does not exist')
            return {"error": "File not found", "path": abs_file_path}, 404

        try:
            with open(abs_file_path, "r", encoding="utf-8") as file:
                content = file.read()
                logging.info('USER requested file path: %s %s', abs_file_path,'has been read successfully')
